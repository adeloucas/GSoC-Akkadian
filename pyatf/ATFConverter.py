...
class ATFConverter(object):
    ...
    def __init__(self):
        ...
        sumerian =     [(r'([\_][\w\d\.\|\!?\#\(\)\[\]-]+[\_])', '\\1'),
                        (r'([\_][\w\d\.\|\!?\#\(\)\[\]-]+[\.|!#\s-][\w\.\|\!?\#\(\)\[\]-]+[\_])', '\\1'),
                        (r'([\_][\w\d\.\|\!?\#\(\)\[\]-]+[\.|!#\s-][\w\.\|\!?\#\(\)\[\]-]+[\.|!#\s-][\w\.\|\!?\#\(\)\[\]-]+[\_])', '\\1'),
                        (r'([\_][\w\d\.\|\!?\#\(\)\[\]-]+[\.|!#\s-][\w\.\|\!?\#\(\)\[\]-]+[\.|!#\s-][\w\.\|\!?\#\(\)\[\]-]+[\.|!#\s-][\w\.\|\!\#\(\)\[\]-]+[\_])', '\\1'),
                        (r'([\_][\w\d\.\|\!?\#\(\)\[\]-]+[\.|!#\s-][\w\.\|\!?\#\(\)\[\]-]+[\.|!#\s-][\w\.\|\!?\#\(\)\[\]-]+[\.|!#\s-][\w\.\|\!\#\(\)\[\]-]+[\.|!#\s-][\w\.\|\!?\#\(\)\[\]-]+[\_])', '\\1'),
                        (r'([\_][\w\d\.\|\!?\#\(\)\[\]-]+[\.|!#\s-][\w\.\|\!?\#\(\)\[\]-]+[\.|!#\s-][\w\.\|\!?\#\(\)\[\]-]+[\.|!#\s-][\w\.\|\!\#\(\)\[\]-]+[\.|!#\s-][\w\.\|\!?\#\(\)\[\]-]+[\.|!#\s-][\w\.\|\!\#\(\)\[\]-]+[\_])', '\\1')]
                        
                        
        akkadian = [(r'([\_][\.\!\#\(\)\[\]\s\-][\w\.\!\#\(\)\[\]\-]+[\.\!\#\(\)\[\]\s\-][\_])', '\\1'),
                    (r'([\_][\.\!\#\(\)\[\]\s\-][\w\.\!\#\(\)\[\]\-]+[\.\!\#\(\)\[\]\s\-][\w\.\|\!\#\(\)\[\]\-]+[\.\|\!\#\(\)\[\]\s\-][\_])', '\\1'),
                    #particulars
                    #40. _GU₄_ KI-MA [_GU₄_]
                    (r'([\_][\s][\w\d\.\|\!?\#\(\)\[\]-]+[\s][\[][\_])', '\\1'),
                    #37. šum-ma# [_GU₄_] Ù LU [_UDU
                    (r'([\_][\]][\s][\w\d\.\|\!?\#\(\)\[\]-]+[\s][\w\d\.\|\!?\#\(\)\[\]-]+[\s][\[][\_])', '\\1'),
                    #76. ša _DUMU-MEŠ_ ul-du-šum ù lu _LUKUR_ ša _DUMU-MEŠ_
                    #81. lu _ÁRAD_-sú ù lu _GÉME_-sú
                    (r'([\_][\s\-][\w\d\.\|\!?\#\(\)\[\]-]+[\s][\w\d\.\|\!?\#\(\)\[\]-]+[\s][\w\d\.\|\!?\#\(\)\[\]-]+[\s][\_])', '\\1')]
